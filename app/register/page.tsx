...
export default function RegisterPage() {
  ...
  const handleRegister = async () => {
    if (!email || !password || !company) { setError('Bitte alle Felder ausfüllen.'); return }
    ...
    <main className="min-h-screen bg-[#f7f6f2] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-[#8a8780] mt-2">14 Tage kostenlos testen</p>
        </div>

        <div className="bg-white border border-[#e0ddd6] rounded-2xl p-8 shadow-sm">
          <h1 className="text-xl font-black tracking-tight text-[#0f0e0c] mb-6">Konto erstellen</h1>
          ...
          <div className="flex flex-col gap-4">
            <div>
              <label className="block text-xs font-mono text-[#8a8780] uppercase tracking-wider mb-1.5">Unternehmen</label>
              <input
                type="text"
                value={company}
          ...
          <p className="text-xs text-[#8a8780] text-center mt-4 leading-relaxed">
            Mit der Registrierung akzeptieren Sie unsere{' '}
            <Link href="/nutzungsbedingungen" className="underline hover:text-[#0f0e0c]">Nutzungsbedingungen</Link>
            {' '}und{' '}
            <Link href="/datenschutz" className="underline hover:text-[#0f0e0c]">Datenschutzerklärung</Link>.
          </p>

          <div className="mt-5 pt-5 border-t border-[#e0ddd6] text-center text-sm text-[#8a8780]">
            Bereits ein Konto?{' '}
            <Link href="/login" className="text-[#1a472a] font-semibold hover:underline">
              Anmelden
            </Link>
          </div>
        </div>
